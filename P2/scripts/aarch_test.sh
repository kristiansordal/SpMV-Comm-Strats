...
if [[ -f "$GROUP_FILE" && -r "$GROUP_FILE" ]]; then
    while IFS= read -r line; do
        ...
        sh launch.sh "aarchq" "$line" 1 1 64
        sh launch.sh "aarchq" "$line" 2 1 64
        sh launch.sh "aarchq" "$line" 3 1 64
        sh launch.sh "aarchq" "$line" 4 1 64
        sh launch.sh "aarchq" "$line" 5 1 64
        sh launch.sh "aarchq" "$line" 6 1 64
        sh launch.sh "aarchq" "$line" 7 1 64
        sh launch.sh "aarchq" "$line" 8 1 64
